...
func parseDirective(input string) (*Directive, error) {
	...
	if !strings.HasPrefix(raw, "%") {
		return nil, fmt.Errorf("sqltmpl: directive %q must start with %%", raw)
	}
	...
	case strings.HasPrefix(normalized, "if"):
		...
		if exprText == "" {
			return nil, fmt.Errorf("sqltmpl: parse directive %q: missing if expression", normalized)
		}
		return &Directive{
			If: &IfDirective{
				Keyword: "if",
	...
	default:
		return nil, fmt.Errorf("sqltmpl: parse directive %q: unsupported directive", normalized)
	}
}
