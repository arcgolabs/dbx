...
- single-column key lookups should prefer `repo.By(typedColumn)` over `any` IDs
- composite-key lookups should prefer typed key sets over `repository.Key` maps
...
- validation hooks
- typed execution helpers for handwritten SQL through `sqlstmt.For[P]`, `sqlstmt.NewTyped[P]`, and `sqlexec.*Typed`
