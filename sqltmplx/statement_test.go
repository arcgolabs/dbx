...
func TestEngineAnalyzeAndCheck(t *testing.T) {
	...
	require.Equal(t, sqltmplx.CheckStageOK, report.Stage)
...
func TestEngineCheckReportsCompileStage(t *testing.T) {
	...
	require.Equal(t, "sqlite", report.Dialect)
	require.Equal(t, sqltmplx.CheckStageCompile, report.Stage)
...
`),
	...
	require.Equal(t, sqltmplx.CheckStageOK, first.Stage)
	...
	require.Equal(t, sqltmplx.CheckStageRender, second.Stage)
...
func TestRegistryCheckMissingTemplateReportsLoadStage(t *testing.T) {
	registry := sqltmplx.NewRegistry(fstest.MapFS{}, sqlite.New())

	report, err := registry.Check("sql/missing.sql", nil)
	require.Error(t, err)
	require.Equal(t, sqltmplx.CheckStageLoad, report.Stage)
